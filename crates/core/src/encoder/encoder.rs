...
use crate::petri_net::pn::{Place, Transition, PetriNet};
...
/// T = {t}
/// F = {(e, t), (t, e')}
fn encode_task(generator: FreshIdGen, e: &ControlFlow, e_prime: &ControlFlow) -> (FreshIdGen, PetriNet) {
...
fn encode_and_split(
    ...
    let net = outputs.iter().fold(
        PetriNet::new().arc_pt(e.id(), t.clone()),
        |net, e_prime| net.arc_tp(t.clone(), e_prime.id()),
    );
    (generator, net)
}
...
fn encode_element(generator: FreshIdGen, element: &ChoreographyEl) -> (FreshIdGen, PetriNet) {
    match element {
        ...
        ChoreographyEl::XorSplit { .. }
        | ChoreographyEl::XorJoin { .. }
        | ChoreographyEl::OrSplit { .. }
        | ChoreographyEl::OrJoin { .. } => {
            todo!("encoding xor/or")
        }
    }
}

fn encode_choreography(generator: FreshIdGen, choreography: &Choreography) -> (FreshIdGen, PetriNet) {
    choreography
        .elements
        .iter()
